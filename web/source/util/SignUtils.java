...
import com.xl.pay.config.SwiftpassConfig;
...
public class SignUtils {
    ...
    public static Map<String, String> checkSignAndReturnStatu(String responseStr) throws BaseException {
        Map<String, String> map = null;
        try {
            map = XmlUtils.toMap(responseStr.getBytes(), "utf-8");
        } catch (Exception e) {
            throw new BaseException(500, "不能解析返回数据");
        }
        if (!SignUtils.checkParam(map, SwiftpassConfig.getKey())) {
            throw new BaseException(500, "签名不正确");
        }
        String status = map.get("status");
        if (!"0".equals(status)) {
            throw new BaseException(500, map.get("message"));
        } else if (!"0".equals(map.get("result_code"))) {
            throw new BaseException(500, "错误编码:" + map.get("err_code") + "错误信息:" + map.get("err_msg"));
        }
        return map;
    }
